...
(function (global, factory) {
    ...
    (function (global, factory) {
      ...
      */
      ...
      const stripMatchingBraces = str => {
        ...
        while (str.match(/(^|(?:(?!\\)[\s\S]))\{[\s\S]*?((?:(?!\\)[\s\S]))\}/)) {
          str = str.replace(/(^|(?:(?!\\)[\s\S]))\{([\s\S]*?)((?:(?!\\)[\s\S]))\}/, '$1$2$3');
        }
      ...
      const entries = (str, options) => {
        let items = parse(str, options),
            entries = [],
            strings = _objectSpread2({}, STRINGS),
          ...
          if (datatype === 'number') {
            ...
            return value.map((_ref) => {
              ...
              return evaluate(datatype, value);
            }).join('');
  ...
  var parser = /*@__PURE__*/getDefaultExportFromCjs(bibtexParse);
